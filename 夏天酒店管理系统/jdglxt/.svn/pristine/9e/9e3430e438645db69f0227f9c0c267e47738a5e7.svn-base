...
public class UpdateClientInfo extends JPanel {
	...
	private String[] money = new String[] {"￥100","￥180","￥220","￥320","￥480","￥600"};
	/**时间格式*/
	SimpleDateFormat sdf = new SimpleDateFormat("yyyy/MM/dd");
	
	public UpdateClientInfo(Roomer a) {
		JPanel jj=this;
		this.setLayout(null);
		...
		// 标题
		JLabel label1 = new JLabel("修改房客信息");
		label1.setBounds(330, 0, 150, 100);
		...
		Canvas_name.setFont(new Font("黑体", Font.BOLD, 20));
		...
		Canvas_contact.setFont(new Font("黑体", Font.BOLD, 20));
		...
		Canvas_roomNo.setFont(new Font("黑体", Font.BOLD, 20));
		...
		Canvas_roomPrice.setFont(new Font("黑体", Font.BOLD, 20));
		...
		for(int i = 0; i < str1.length; i++) {
			if(str1[i].equals(a.getRoomType())) {
		...
		jcb.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent e) {
				...
				for (int i = 0 ; i < str1.length; i++) {
					if(str1[i].equals(s)) {
						Canvas_roomPrice.setText(money[i]);
						return;
					}
				}
			}
		...
		Canvas_firstTime.setFont(new Font("黑体", Font.BOLD, 20));
		...
		Canvas_id.setFont(new Font("黑体", Font.BOLD, 20));
...
//					}
//				}
				String roomPrice = new RoomerViewDao().findById(roomNo).getRoomPrice().toString();
//				Canvas_roomPrice.setText(roomPrice);
				...
				Roomer aa = new Roomer(RoomerAccount.accountNum,name, contact, roomNo, roomType, roomPrice, firstTime, lastTime, dayNum, inTime,id,"已入住");
				
				
				boolean b = new RoomerDao().add(aa);
				boolean c = new RoomerViewDao().update("已入住",aa.getRoomNo() );
				
				if(b==true&&c==true) {
					new RoomerViewDao().update("维护中",a.getRoomNo() );
					new RoomerDao().deleteById(a);
