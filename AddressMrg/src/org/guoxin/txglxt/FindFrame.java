...
public class FindFrame extends JFrame {
	...
	/** 标题1标签 */
	private JLabel label1 = new JLabel("查找");
	/** 标题2标签 */
	...
	/** 查询全部按钮 */
	private JButton all = new JButton("全部");
	/** 确定按钮 */
	...
	public FindFrame(JFrame jf, String str) {
		...
		text[0] = new JTextField("请输入需要查找的用户姓名");
		info[0].setFont(f);
		...
		for (int i = 0; i < 7; i++) {
			text[i].addKeyListener(new KeyListener() {
				...
				@Override
				public void keyPressed(KeyEvent e) {
					if (e.getKeyChar() != KeyEvent.VK_ENTER && text[0].getText().equals("请输入需要查找的用户姓名")) {
						text[0].setText("");
						text[0].requestFocus();
					}
					if (e.getKeyChar() == KeyEvent.VK_ENTER) {
		...
		text[0].addMouseListener(new MouseListener() {
			...
			public void mousePressed(MouseEvent e) {
				if (text[0].getText().equals("请输入需要查找的用户姓名")) {
					text[0].setText("");
				}
			}
